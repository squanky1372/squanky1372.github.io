...
io.on('connection', (socket) => {
  ...
  socket.on('fetch', msg => {
    var msg_rec = msg.toString()
    console.log('Fetching file:', msg_rec)
    var msg_array = msg_rec.split("!@#$%")
    var text_file = get_txt_file(msg_array[0], msg_array[1]);
    console.log(text_file)
    
    //emit text file (or failure) to listening webpages
    if(text_file == -1) io.emit("fetch failure", text_file)
    else io.emit('fetch response', text_file, msg_array[0], msg_array[1]);
  });
});
